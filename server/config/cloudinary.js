...
// Admission Application Image Storage (for applicant photo)
const admissionApplicationStorage = new CloudinaryStorage({
  cloudinary: cloudinary,
  params: {
    folder: "fgdc-kohat/admissions",
    allowed_formats: ["jpg", "jpeg", "png", "webp"],
    transformation: [
      {
        width: 600,
    ...
    public_id: (req, file) => {
      const timestamp = Date.now();
      const randomNum = Math.round(Math.random() * 1e9);
      return `admission-${timestamp}-${randomNum}`;
    },
  },
});

export const admissionApplicationUpload = multer({
  storage: admissionApplicationStorage,
  limits: {
